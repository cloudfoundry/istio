...
func ConvertInstances(serviceEntry *networking.ServiceEntry, namespace string, creationTime time.Time) []*model.ServiceInstance {
		...
		for _, serviceEntryPort := range serviceEntry.Ports {
			if len(serviceEntry.Endpoints) == 0 &&
				...
				for _, endpoint := range serviceEntry.Endpoints {
					instances = append(instances, convertEndpoint(service, serviceEntryPort, endpoint))
				}
			}
		}
	}
